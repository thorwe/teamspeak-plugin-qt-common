...
namespace thorwe {
    ...
    public:
        std::pair<T*, bool> add_item(ts::connection_id_t connection_id, ts::client_id_t client_id, std::unique_ptr<T> new_item)
        {
        ...
        // internal use
        T* get(ts::connection_id_t connection_id, ts::client_id_t client_id)
        {
        ...
        template<typename U>
        void do_for_each(U&& fn, ts::connection_id_t connection_id = 0)
        {
            if (connection_id == 0)
            {
    ...
    private:
        using ConnectionMap = std::unordered_map<ts::client_id_t, std::unique_ptr<T>>;
        std::unordered_map<ts::connection_id_t, ConnectionMap> m_storage;
    };
    ...
    public:
        std::pair<T*, bool> add_item(ts::connection_id_t connection_id, ts::client_id_t client_id, std::unique_ptr<T> item)
        {
        ...
        bool contains(ts::connection_id_t connection_id, ts::client_id_t client_id)
        {
